...
func handleWorkerAdd(w http.ResponseWriter, r *http.Request, config utils.ManagerConfig, db *sql.DB) {
	oauthKey := r.Header.Get("Authorization")
...
func handleWorkerRMName(w http.ResponseWriter, r *http.Request, config utils.ManagerConfig, db *sql.DB) {
	oauthKey := r.Header.Get("Authorization")
	if incorrectOauth(oauthKey, config.OAuthToken) {
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return
